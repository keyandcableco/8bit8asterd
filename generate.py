...
__TRANSPORT_UI__
  ...
  <section class="section" id="seqSection">
    ...
    <div class="module">
      <h2>MIDI File</h2>
      <div class="bar filepick" style="margin:0 13px 10px;">
...
function harpScaleFor(){
  ...
  if (m.kind === 'chord'){
    ...
    if (!sp) return null;
    return { tones: sp.tones.length === 3 ? sp.tones.concat([9]) : sp.tones,
             root: chordRoot };
  }
  if (m.kind === 'key'){
  ...
  if (m.kind === 'perChord' || m.kind === 'perChordPent'){
    ...
    if (!key) return null;
    const table = m.kind === 'perChordPent' ? CHORD_PENT
