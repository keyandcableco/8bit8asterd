...
# Patches kept out of the normal list because they sit at the far end of
# several controls at once. Not unstable in the sense of crashing -- the
# clamps still hold -- but loud, detuned and hard to play.
HIDDEN_PRESETS = {
    "Dead Air":     dict(clock_enable=1, clock_drop=63, clock_hold=63,
                         warp_mode=7, warp_rate=90, warp_depth=55),
    "Wasp Factory": dict(warp_mode=10, warp_rate=120, warp_depth=63,
                         wave_enable=1, wave_shape=4, wave_level=15),
    "Cathedral":    dict(draw_enable=1, draw_a=1, draw_a_level=15,
                         draw_b=4, draw_b_level=12,
                         buzz_enable=1, buzz_pure=1, buzz_ratio=2),
    "Wrong Speed":  dict(clock_enable=1, clock_drop=50, clock_hold=20,
                         clock_sweep=3, uni_enable=1, uni_detune=26),
    "Hornet":       dict(warp_mode=10, warp_rate=120, warp_depth=63,
                         buzz_enable=1, buzz_ratio=1, buzz_detune=6,
                         draw_enable=1, draw_a=8, draw_a_level=9),
}
...
__EXTRA_HEAD__
<link rel="icon" type="image/svg+xml" href="data:image/svg+xml;base64,">
<link rel="icon" type="image/png" sizes="32x32" href="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAAe0lEQVR42u2XQQrAMAgEdekT7A/7rPyw/qE9FdIQijlUL+stEJmBBWFVhjGzS34cd9f+jUz4jIFM+IyFbPgooVH42XwJsB8W+gcpHgps0cyjma7uMwIKlAvwElKAl5ACFOAlpIBWFZOnJ2JWGLPgrwgyJXoWvqpzRj2/AanFOPGtYpTEAAAAAElFTkSuQmCC">
</head>
<body>
<div class="rack">
...
__TRANSPORT_UI__
  ...
  <section class="section" id="seqSection">
    ...
    <div class="module">
      <h2>Password</h2>
      <div class="bar cc-row" style="margin:0 13px 10px;align-items:flex-end;">
        <div class="cc">
          <input type="text" id="pwInput" placeholder="ENTER CODE"
                 spellcheck="false" style="width:220px;font-family:var(--mono)">
          <span class="cc-label">Enter</span>
        </div>
        <div class="cc">
          <button class="btn" id="pwGo">Go</button>
          <span class="cc-label">Load patch</span>
        </div>
        <span class="spacer"></span>
        <div class="cc">
          <input type="text" id="pwShow" readonly placeholder="—"
                 style="width:220px;font-family:var(--mono)">
          <span class="cc-label">This patch</span>
        </div>
        <div class="cc">
          <button class="btn" id="pwMake">Make</button>
...
function buildMidiFile(){
  ...
  // The code, on the keyboard. Ignored while typing into a field, or it
  // would fire while someone enters a password.
  ...
  if (pwGo && pwIn){
    ...
    pwIn.onkeydown = e => { if (e.key === 'Enter') pwGo.onclick(); };
  }
  const pwMake = document.getElementById('pwMake');
  if (pwMake && pwOut) pwMake.onclick = () => {
    pwOut.value = pwEncode();
    pwOut.select && pwOut.select();
    log('sys', 'Password for the current patch. Settings only -- no sequence.');
  };

  const saveBtn = document.getElementById('imgSave');
  if (saveBtn) saveBtn.onclick = saveImage;

  // MPE input. The emulator remaps its controller's channels into lanes, so it
  // owns the switch there; the hardware panel only has to tell the firmware.
  const mpeSw = document.getElementById('mpeSw');
  if (mpeSw) mpeSw.onclick = () => {
    const on = !mpeSw.classList.contains('active');
    if (typeof setMpe === 'function') { setMpe(on); return; }
    mpeSw.classList.toggle('active', on);
    send('MPE:' + (on ? 1 : 0));
  };
...
/* ==== Cheat code and patch passwords ==================================== */
// Two ways in. The code unlocks a bank of patches kept out of the normal
// list, and on the emulator lifts the master clock's limits. A password
// carries a patch alone -- no sequence, no file -- short enough to read
// aloud or print on a card.

const HIDDEN_PRESETS = __HIDDEN_PRESETS__;
const UNSAFE_CLOCK = __UNSAFE_CLOCK__;
let unlocked = false;

// Up up down down left right left right B A, on the keyboard or a gamepad.
const KONAMI = ['ArrowUp','ArrowUp','ArrowDown','ArrowDown',
                'ArrowLeft','ArrowRight','ArrowLeft','ArrowRight','b','a'];
let konamiAt = 0;

function konamiStep(token){
  konamiAt = (token === KONAMI[konamiAt]) ? konamiAt + 1
           : (token === KONAMI[0] ? 1 : 0);
  if (konamiAt >= KONAMI.length){ konamiAt = 0; unlock(); }
}
...
function unlock(){
  ...
  if (sel){
    ...
    sep.disabled = true; sep.textContent = '\u2014 unlocked \u2014';
    sel.appendChild(sep);
    Object.keys(HIDDEN_PRESETS).forEach(name => {
      const o = document.createElement('option');
      o.value = name; o.textContent = name;
      sel.appendChild(o);
      PRESETS[name] = HIDDEN_PRESETS[name];
    });
  }

  // The clock's limits exist to keep a forty-year-old chip inside its rating.
  // Lifting them is only offered where there is no chip to damage.
  if (typeof UNSAFE_CLOCK !== 'undefined' && UNSAFE_CLOCK){
    send('XCLK:1');
    document.body.classList.add('unlocked');
  }

  log('sys', 'Unlocked: ' + Object.keys(HIDDEN_PRESETS).length +
      ' patches added to the preset list' +
      ((typeof UNSAFE_CLOCK !== 'undefined' && UNSAFE_CLOCK)
        ? ', and the master clock limits are off.' : '.'));
}

/* ---- patch passwords ---- */
// Crockford base32: no I, L, O or U, so nothing is mistaken for 1 or 0 when
// read aloud or copied off a screen. Only the parameters that differ from
// Init are carried, which is what keeps a code short: a dozen changes rather
// than fifty-seven values.
const PW_ALPHABET = '0123456789ABCDEFGHJKMNPQRSTVWXYZ';
...
function pwEncode(){
  ...
  const deltas = [];
  PARAMS.forEach(p => {
    const init = (PRESETS['Init'] && PRESETS['Init'][p.key] !== undefined)
      ? PRESETS['Init'][p.key] : p.default;
    if (values[p.index] !== init) deltas.push([p.index, values[p.index]]);
  });

  push(deltas.length, 6);
  deltas.forEach(([i, v]) => { push(i, 6); push(v, 8); });
  ...
  for (let i = 0; i < bits.length; i += 5){
    ...
    // Weighted by position, so transposing two characters is caught as well
    // as changing one, and a code's length is part of what it checks.
    sum = (sum + v * pos++) & 31;
    out += PW_ALPHABET[v];
  }
  out += PW_ALPHABET[sum];                 // one check character
  return out.replace(/(.{4})(?=.)/g, '$1-');
}

function pwDecode(code){
  ...
  const chars = clean.slice(0, -1), check = clean.slice(-1);
  let sum = 0, pos = 1;
  const bits = [];
  for (const c of chars){
    const v = PW_ALPHABET.indexOf(c);
    if (v < 0) throw new Error('bad character: ' + c);
    sum = (sum + v * pos++) & 31;
    for (let i = 4; i >= 0; i--) bits.push((v >> i) & 1);
  }
  if (PW_ALPHABET[sum] !== check) throw new Error('check digit does not match');

  let at = 0;
  const take = n => { let v = 0; for (let i = 0; i < n; i++) v = (v << 1) | bits[at++]; return v; };
  const count = take(6);
  if (6 + count * 14 > bits.length) throw new Error('truncated');

  const out = [];
  for (let i = 0; i < count; i++){
    const idx = take(6), val = take(8);
    if (idx >= PARAMS.length) throw new Error('unknown setting in code');
    out.push([idx, val]);
  }
  return out;
}
...
function pwApply(code){
  ...
  // put seventy-seven writes on the wire back to back -- fifty-seven resets
  // and then the deltas -- which a serial link will not all survive.
  PARAMS.forEach(p => {
    values[p.index] = (PRESETS['Init'] && PRESETS['Init'][p.key] !== undefined)
      ? PRESETS['Init'][p.key] : p.default;
  });
  deltas.forEach(([i, v]) => { values[i] = v; });
  renderAll();
  send('LOAD:' + values.join(','));
...
// The code on a controller, which is where it belongs. The detector is the
// same one the keyboard feeds, so either route gets there.
const GP_KONAMI = [[GP.UP,'ArrowUp'], [GP.DOWN,'ArrowDown'],
                   [GP.LEFT,'ArrowLeft'], [GP.RIGHT,'ArrowRight'],
                   [GP.B,'b'], [GP.A,'a']];

function gpKonami(hit){
  for (const [btn, token] of GP_KONAMI) if (hit(btn)) konamiStep(token);
}
...
def emit_html(path, transport="serial", extra_head="", extra_body=""):
    ...
    html = (HTML_TEMPLATE
            ...
            .replace("__HIDDEN_PRESETS__", json.dumps(HIDDEN_PRESETS))
            .replace("__UNSAFE_CLOCK__", "true" if transport == "wasm" else "false"))
    with open(path, "w") as f:
        f.write(html)
